...
def transform_augment_cd_all(img_A, img_B, label=None, flow=None, split='val', min_max=(-1, 1)):
    """
    Applies same augmentation to all inputs: pre-image, post-image, label, and flow.
    """
    ...
    if split == 'train':
        ...
        if vflip:
            ...
            if flow is not None:
                ...
                flow[1] *= -1  # negate y-direction

    img_A = img_A * (min_max[1] - min_max[0]) + min_max[0]
    img_B = img_B * (min_max[1] - min_max[0]) + min_max[0]

    if label is not None:
        label = label.clamp(0, 1)
        if label.dim() > 2:
            label = label[0]  # Squeeze extra channel

    return img_A, img_B, label, flow
