...
#        F_A.append(f_A_r)
#        F_B.append(f_B_r)
#    device = torch.device(f"cuda:{opt['gpu_ids'][0]}" if opt['gpu_ids'] else "cuda")
    for t_i in range(0, len(cd_model_opt["t"])):
        F_A.append([f.clone().cuda() for f in f_A_r])
        F_B.append([f.clone().cuda() for f in f_B_r])
    if isinstance(netCD, torch.nn.DataParallel):
        netCD = netCD.module
